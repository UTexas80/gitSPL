...
colnames(means) <- c("Signal", "Pct Win", "Mean Win", "Mean Loss")
colnames(medians) <- c("Signal", "Median Win", "Median Loss")

colnames(sums) <- c("Signal", "Number Trades") 
...
#png(filename = "20090501_rsi2_performance.png", 720, 720)
...
# Calculate rule returns
ret_up <- ret * sigup
colnames(ret_up) <-'Long System Returns'
ret_dn <- ret * sigdn
colnames(ret_dn) <- 'Short System Returns'
ret_all <- ret * sig
colnames(ret_all) <- 'Total System Returns'

# Calculate performance statistics
# png(filename = "20090430_rsi2_performance_updated.png", 720, 720)
charts.PerformanceSummary(cbind(ret_up,ret_dn), methods = 'none',
  main = 'RSI(2) Performance - RSI steps = 1-, Size steps = 0.30')
dev.off()
