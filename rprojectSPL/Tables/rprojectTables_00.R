...
ult.osc <- ultimateOscillator(SPL[,c("high","low","close")]) 
...
tblWPR <- data.frame(date,"spl",wpr)                                            # Create William's %R Indicator Table
...
tblWPR<-tblWPR[complete.cases(tblWPR),]                                         # Delete William's %R Indicator NA
# Moving Average Tables
...
colnames(tblMOM_stoch) <- c("date", "symbol", "fastK", "fastD", "slowD")        # Rename tblStoch Table Columns
...
colnames(tblWPR) <- c("date", "symbol", "wpr" )                                 # Rename tblWPR Table Columns
# Moving Average Tables
